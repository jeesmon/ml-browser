...
import com.jeesmon.apps.mlbrowser.R;
...
public class ApplicationUtils {
	...
	private static String mFontFixString = null;
	private static String mFontFixType = null;
	...
	public static String getFontFixString(Context context, String fixType) {
		if (mFontFixString == null || !fixType.equals(mFontFixType)) {
			InputStream is = fixType.equals(Constants.PREFERENCES_ML_FONTFIX_ALT) ? 
					context.getResources().openRawResource(R.raw.font_fix_ml_alt) : context.getResources().openRawResource(R.raw.font_fix_ml);
			if (is != null) {
				StringBuilder sb = new StringBuilder();
				String line;
				...
				try {
					...
					Log.w("FontFix", "Unable to load FontFix: " + e.getMessage());
				} finally {
					try {
						is.close();
					} catch (IOException e) {
				...
				mFontFixString = sb.toString();
			} else {        
				mFontFixString = "";
			}
			mFontFixType = fixType;
		}
		return mFontFixString;
	}
